...
async def home(request):
    return templates.TemplateResponse('index.html', request=get_custom_filters())
...
# async def convert(request: Request):
    ...
    #load the template
    try:
        jinja2_env = jinja2_env.from_string(Request.form['template'])
    except (exceptions.TemplateSyntaxError, exceptions.TemplateError) as e:
        return "Syntax error in jinja2 template: {0}".format(e)

    dummy_values = ['Lorem', 'Amet', 'Elit']
    values = {}
